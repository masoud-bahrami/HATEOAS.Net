...
namespace HATEOAS.Net.HAL
{
    ...
    {
        public EmbeddedResource WithEditLink(Link link)
        {
            LinksObjects.Add(new LinkObject(LinkRelations.Edit) { Links = new List<Link> { link } });
            return this;
        }
        public EmbeddedResource WithEditLink(string href,string httpVerb, bool? templated = null, string name = "", string type = "", string deprecation = "")
        {
            LinksObjects.Add(new LinkObject(LinkRelations.Edit)
            {
                Links = new List<Link>
                {

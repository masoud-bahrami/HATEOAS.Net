...
namespace HATEOAS.Net.HAL
{
    ...
    {
        public EmbeddedResource WithSelfLink(Link link)
        {
            LinksObjects.Add(new LinkObject(LinkRelations.Self) { Links = new List<Link> { link } });
            return this;
        }
        public EmbeddedResource WithSelfLink(string href, string httpVerb, bool? templated = null, string name = "", string type = "", string deprecation = "")
        {
            LinksObjects.Add(new LinkObject(LinkRelations.Self)
            {
                Links = new List<Link>
                {

...
namespace HATEOAS.Net.HAL
{
    ...
    {
        public EmbeddedResource WithStartLink(Link link)
        {
            LinksObjects.Add(new LinkObject(LinkRelations.Start) { Links = new List<Link> { link } });
            return this;
        }
        public EmbeddedResource WithStartLink(string href, string httpVerb, bool? templated = null, string name = "", string type = "", string deprecation = "")
        {
            LinksObjects.Add(new LinkObject(LinkRelations.Start)
            {
                Links = new List<Link>
                {

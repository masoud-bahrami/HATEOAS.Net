...
namespace HATEOAS.Net.HAL
{
    ...
    {
        public HAL WithFirstLink(Link link)
        {
            linkObjects.Add(new LinkObject(LinkRelations.First) { Links = new List<Link> { link } });
            return this;
        }
        public HAL WithFirstLink(string href, string httpVerb, bool? templated = null, string name = "", string type = "", string deprecation = "")
        {
            linkObjects.Add(new LinkObject(LinkRelations.First)
            {
                Links = new List<Link>
                {

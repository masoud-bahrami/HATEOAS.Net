...
namespace HATEOAS.Net.HAL
{
    ...
    {
        public HAL WithLastLink(Link link)
        {
            linkObjects.Add(new LinkObject(LinkRelations.Last) { Links = new List<Link> { link } });
            return this;
        }
        public HAL WithLastLink(string href, string httpVerb, bool? templated = null, string name = "", string type = "", string deprecation = "")
        {
            linkObjects.Add(new LinkObject(LinkRelations.Last)
            {
                Links = new List<Link>
                {

...
namespace HATEOAS.Net.HAL
{
    ...
    {
        public HAL WithNextLink(Link link)
        {
            linkObjects.Add(new LinkObject(LinkRelations.Next) { Links = new List<Link> { link } });
            return this;
        }
        public HAL WithNextLink(string href, string httpVerb, bool? templated = null, string name = "", string type = "", string deprecation = "")
        {
            linkObjects.Add(new LinkObject(LinkRelations.Next)
            {
                Links = new List<Link>
                {

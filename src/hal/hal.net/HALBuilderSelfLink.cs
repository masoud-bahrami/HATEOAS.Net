...
namespace HATEOAS.Net.HAL
{
    ...
    {
        public HAL WithSelfLink(Link link)
        {
            linkObjects.Add(new LinkObject(LinkRelations.Self) { Links = new List<Link> { link } });
            return this;
        }
        public HAL WithSelfLink(string href, string httpVerb, bool? templated = null, string name = "", string type = "", string deprecation = "")
        {
            linkObjects.Add(new LinkObject(LinkRelations.Self)
            {
                Links = new List<Link>
            {

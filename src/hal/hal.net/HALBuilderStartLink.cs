...
namespace HATEOAS.Net.HAL
{
    ...
    {
        public HAL WithStartLink(Link link)
        {
            linkObjects.Add(new LinkObject(LinkRelations.Start) { Links = new List<Link> { link } });
            return this;
        }
        public HAL WithStartLink(string href, string httpVerb, bool? templated = null, string name = "", string type = "", string deprecation = "")
        {
            linkObjects.Add(new LinkObject(LinkRelations.Start)
            {
                Links = new List<Link>
            {
